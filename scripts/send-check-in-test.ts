 ...
 *   npx tsx --env-file=.env.local scripts/send-check-in-test.ts [email] [name] [ios|android] [not_started|active]
 */
import {
...
async function main() {
  ...
  const platform = (process.argv[4] === "android" ? "android" : "ios") as
    | "ios"
    | "android";
  const audience = process.argv[5] === "active" ? "active" : "not_started";
